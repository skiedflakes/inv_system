<?php
	...
	while($row = mysqli_fetch_array($data)){
		...
		if($is_discounted == 0){
			...
			$disc_price = 0;
			$final_price = $row["gross_price"];
		}else{
			$vat_price = $row["is_vatable"] == 1?$row["gross_price"]*0.12:0;
			$disc_price = $discount != 0?$row["gross_price"]*$discount:0;
			$final_price = $row["gross_price"]-$disc_price;
		}
